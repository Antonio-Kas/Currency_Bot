...
bot = telebot.TeleBot("7823886815:AAHdzHHdG9nvmUuocjSpeBKliQkHGZNhFQM")
currency = CurrencyConverter()
amount = 0

@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(message.chat.id, "Привіт, введіть сумму")
...
def summa(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id,'Неправильне введення. Впишіть суму')
        bot.register_next_step_handler(message, summa)
        return
...
def callback(call):
    if call.data != 'else':
        ...
        bot.send_message(call.message.chat.id, 'Введите пару значенний через слэш')
